...
class Generator(nn.Module):
    ...
    def execute(self, latents_in, depth, alpha, labels_in=None):
        ...
        # print("generator output", fake_images.is_stop_grad())
        return fake_images
...
class Discriminator(nn.Module):
    ...
    def execute(self, images_in, depth, alpha=1., labels_in=None):
        ...
        if self.structure == 'fixed':
            ...
            raise KeyError("Unknown structure: ", self.structure)
        # print("discriminator output", scores_out.is_stop_grad())
        return scores_out
...
class StyleGAN:
    ...
    def __init__(self, structure, resolution, num_channels, latent_size,
                 ...
                 d_repeats=1, use_ema=False, ema_decay=0.999, device=None):
        """
    ...
    def __progressive_down_sampling(self, real_batch, depth, alpha):
        ...
        if depth > 0:
            ...
            prior_ds_real_samples = nn.interpolate(nn.Pool(prior_down_sample_factor)(real_batch), scale_factor=2, mode='nearest')
        else:
